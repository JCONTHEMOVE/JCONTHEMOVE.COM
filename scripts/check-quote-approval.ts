...
import { approveQuoteRevision, ensureQuoteRevisionInfrastructure } from '../server/services/quoteRevisions';
...
try {
  ...
  await database.query('UPDATE quote_revisions SET subtotal=200,final_pre_tax_total=200,customer_total=200 WHERE id=$1', [quoteId]);
  const input = { quoteId, actor: { userId: 'owner', isOwner: true, canApproveStandard: true },
  ...
  assert.equal((await database.query('SELECT status FROM quote_revisions')).rows[0].status, 'draft');
