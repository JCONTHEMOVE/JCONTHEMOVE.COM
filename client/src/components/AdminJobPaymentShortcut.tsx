...
type Snapshot = {
  ...
  rewards: {
    paymentWorkflow?: "legacy" | "canonical";
...
function fingerprint(snapshot: Snapshot) {
  ...
  return JSON.stringify({
    ...
    paymentWorkflow: rewards.paymentWorkflow,
  });
}

function unavailable(snapshot: Snapshot, action: Action) {
  const { lead, rewards } = snapshot;
  // A missing capability fails closed during mixed-version deployments.
  if (rewards.paymentWorkflow !== "legacy") return "Payment options are unavailable for the current payment workflow. Refresh the job or use payment reconciliation.";
...
export function AdminJobPaymentShortcut({ leadId }: { leadId: string }) {
  ...
  const query = useQuery<Snapshot>({
    ...
    queryFn: async () => {
      ...
      const [lead, employees, rewards] = await Promise.all([
  ...
  const submit = async () => {
    ...
    try {
      ...
      setReview(null); setConfirmed(false);
      for (const key of [["/api/leads"], ["/api/jobs/planner"], ["/api/rewards"], ["/api/rewards/lead", leadId], ["admin-payment-shortcut", leadId]]) {
