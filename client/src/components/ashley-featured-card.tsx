...
export function AshleyFeaturedCard(){
  ...
  return <aside aria-label="Ashley's Shop" className="min-w-0 rounded-2xl border border-rose-400/30 bg-background/95 p-4">
    ...
    {item?<Link href={`/handmade-jewels-by-ashley/${encodeURIComponent(item.id)}`} className="mt-3 flex min-w-0 items-center gap-3 rounded-lg focus-visible:outline focus-visible:outline-2">
      <CatalogImage src={item.image_url} alt={item.title} className="h-20 w-20 shrink-0 rounded-xl object-cover"/>
