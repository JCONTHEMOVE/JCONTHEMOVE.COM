...
export function trainingThankYou(displayName:string,scenarioId:string,verifiedAmount?:number){
  const name=displayName.replace(/[\r\n<>@*_`~\\]/g,'').slice(0,80)||'A coworker';
  if(verifiedAmount!==undefined)return {username:'JC ON THE MOVE',content:verifiedAmount>0?`Owner verified ${name}'s contribution on ${scenarioId}: +${verifiedAmount} JCMOVES credited (${Math.min(100,verifiedAmount)} contribution + ${Math.max(0,verifiedAmount-100)} bonus). Thank you for helping the team! https://www.jconthemove.com/crew/pricing-training`:`Owner review completed for ${scenarioId}. Check your private training page for feedback: https://www.jconthemove.com/crew/pricing-training`,allowed_mentions:{parse:[]}};
...
export async function thankTrainingContributor(name:string,scenarioId:string,verifiedAmount?:number):Promise<'sent'|'unconfigured'|'failed'|'uncertain'>{
  ...
  try{const response=await fetch(url,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(trainingThankYou(name,scenarioId,verifiedAmount)),signal:AbortSignal.timeout(7000),redirect:'error'});return response.ok?'sent':response.status>=500?'uncertain':'failed';}
