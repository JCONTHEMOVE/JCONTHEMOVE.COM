...
export async function getJobPaymentReconciliation(leadId: string) {
  ...
  try {
    ...
    const job = jobs[0];
    const { rows: sums } = await client.query<{ paid: string; gift: string; count: string }>(
      `SELECT COALESCE(SUM(amount_cents),0)::text AS paid,COALESCE(SUM(gift_funded_cents),0)::text AS gift,
        COUNT(*)::text AS count FROM job_confirmed_payments WHERE lead_id=$1`, [leadId]);
    ...
    return { enabled: true as const, leadId, status: job.status, quoteRevisionId: job.quote_id,
      ...
      refundReconciliationAvailable: false as const, automaticRewardsEnabled: false as const };
  } catch (error) {
