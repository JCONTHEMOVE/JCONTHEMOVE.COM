...
export function ensurePhoneRewards() {
  return ready ??= pool.query(`
    CREATE TABLE IF NOT EXISTS phone_rewards_members (
      phone TEXT PRIMARY KEY, user_id VARCHAR NOT NULL REFERENCES users(id),
...
export async function findPhoneRewardsCustomer(phone: unknown): Promise<{ id: string; email: string | null } | null> {
  const normalized = rewardsPhone(phone);
  if (!normalized) return null;
  await ensurePhoneRewards();
  const result = await pool.query(`SELECT u.id, u.email FROM phone_rewards_members m
