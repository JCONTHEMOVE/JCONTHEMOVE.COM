...
3. Coordinate quote/completion writers with reward settlement, including newly approved quote revisions and assignments changing during retries.
   Canonical final invoices now use stable customer/order/invoice/publish keys derived from the closeout and final quote, and retain a persisted due date across approval retries. Local persistence reuses a provider invoice without resetting its status and rejects changed financial bindings. Key/persistence and closeout retry tests passed; actual provider timeout recovery and payments arriving during invoice creation still require acceptance.
