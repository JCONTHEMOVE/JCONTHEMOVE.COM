...
3. Coordinate quote/completion writers with reward settlement, including newly approved quote revisions and assignments changing during retries.
   Canonical final invoices now use stable customer/order/invoice/publish keys derived from the closeout and final quote, and retain a persisted due date across approval retries. Local persistence reuses a provider invoice without resetting its status and rejects changed financial bindings. Key/persistence and closeout retry tests passed; actual provider timeout recovery remains open. Late-payment publication is now a reproduced defect, not merely unverified: see the diagnostic below.
...
### Open-defect diagnostic: payment during final invoice creation

```text
node --import tsx scripts/check-quote-approval.ts <pglite-dist-index.js> --reproduce-late-final-invoice
```

This opt-in diagnostic prints `REPRODUCED OPEN RELEASE BLOCKER` when it confirms the unsafe behavior. A zero exit is successful reproduction, **not release acceptance**; it is deliberately excluded from the normal CI invocation. September 9 local Node 24/PGlite execution reproduced it and completed the existing quote-approval checks.

The harness approves a $120 final quote with a $30 recorded payment, then invokes the actual invoice service for $90. Its simulated provider commits another $90 through the real payment-ledger function after creating the order and before invoice publication. Publication observes all $120 recorded, but proceeds with $90 and persists that amount locally. Provider methods and invoice storage are injected; unexpected fetches throw. This is a deterministic interleaving on an isolated database, not real Square acceptance or a multi-session concurrency test.
