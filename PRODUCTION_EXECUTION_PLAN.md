...
| Continuous availability | The draft combines readiness and apex/www checks with one incident-service reporter; manual drills use a separate heartbeat. GitHub schedules may be delayed. | Configure the email-first incident service, confirmed owner email, exact Discord destination, separate production/drill heartbeat secrets and independent missed-check coverage. See AVAILABILITY_RELEASE.md; actual receipt and escalation tests remain required. |
